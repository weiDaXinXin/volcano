...
func TestJobTemplateFileDocuments(t *testing.T) {
...
`, server.URL)
					...
					if operation == "create" {
						saved := *createJobTemplateFlags
						t.Cleanup(func() { *createJobTemplateFlags = saved })
						InitCreateFlags(cmd)
						cmd.RunE = func(cmd *cobra.Command, args []string) error { return CreateJobTemplate(cmd.Context()) }
					} else {
						saved := *deleteJobTemplateFlags
						t.Cleanup(func() { *deleteJobTemplateFlags = saved })
						InitDeleteFlags(cmd)
						cmd.RunE = func(cmd *cobra.Command, args []string) error { return DeleteJobTemplate(cmd.Context()) }
					}
					...
					expected := []string{
						"POST " + base + "default/jobtemplates",
						"POST " + base + "team-a/jobtemplates",
					}
					if operation == "delete" {
						expected = []string{
							"DELETE " + base + "default/jobtemplates/" + firstName,
							"DELETE " + base + "team-a/jobtemplates/second",
						}
					}
